...
async function handleMessage(group, consumer, stream, entry) {
    ...
    if (result.success) {
        ...
        if (next >= MAX_RETRIES) {
            ...
            await redis.xack(stream, group, id);
            console.warn(`[DLQ] Event moved to ${DLQ_STREAM}: type=${type} sessionId=${sessionId} msgId=${id} retries=${next}`);
        }
        // Don't ack - message stays pending for XAUTOCLAIM
    }
}
async function runConsumer(group, consumer, readFn) {
...
async function workerLoop(group, consumer, blockMs, count) {
    while (true) {
        try {
            const result = await redis.xreadgroup(STREAM, group, consumer, blockMs, count);
            await runConsumer(group, consumer, async () => result);
        }
        catch (err) {
            console.error(`[Worker ${group}/${consumer}] Read error:`, err?.message);
            await new Promise((r) => setTimeout(r, 1000));
        }
    }
}
function parseAutoclaimEntries(claimed) {
    const entries = [];
    for (const item of claimed) {
        if (Array.isArray(item) && item.length >= 2) {
            const id = String(item[0]);
            const fields = Array.isArray(item[1]) ? item[1] : [];
...
async function autoclaimLoop(group, consumer, intervalMs) {
    while (true) {
        await new Promise((r) => setTimeout(r, intervalMs));
        try {
            const [, claimed] = await redis.xautoclaim(STREAM, group, consumer, PENDING_IDLE_MS, "0");
            const entries = parseAutoclaimEntries(Array.isArray(claimed) ? claimed : []);
            for (const entry of entries) {
                ...
                if (entry.fields.length > 0) {
                    try {
                        ...
                        console.error(`[Worker] Error processing claimed ${entry.id}:`, err?.message);
                        await redis.incrementRetryCount(STREAM, entry.id);
                    }
                }
            }
            ...
            console.error(`[Worker ${group}/${consumer}] Autoclaim error:`, err?.message);
        }
    }
}
