...
async function init() {
  try {
    ...
    console.warn("Redis consumer groups init (optional):", (err as Error)?.message);
  }
}
...
wss.on("connection", async (ws, req) => {
  ...
  try {
    ...
    let voiceProvider: Awaited<ReturnType<typeof createVoiceProvider>> = null;
    ...
    startOrResumeTimer(session.sessionId).then((state) => {
      ...
      console.error("[Timer] startOrResumeTimer error:", (err as Error)?.message);
    });
