 ...
 */
export async function observeCodeAndInject(
  sessionId: string,
  code: string,
  language: string
): Promise<ObserveCodeResult | null> {
  ...
  try {
    ...
    const result = (await res.json()) as ObserveCodeResult;
    ...
    console.error("[Observer] Fetch error:", (err as Error)?.message);
    return null;
  }
}
