...
export async function getSubscriptionTier(sessionId: string): Promise<SubscriptionTier> {
...
/** Internal result that includes subscription tier alongside interview context. */
interface SessionContextWithTier extends SessionContext {
...
const contextCache = new Map<string, { data: SessionContextWithTier | null; ts: number }>();
const CACHE_TTL_MS = 60_000;

async function getSessionContextWithTier(sessionId: string): Promise<SessionContextWithTier | null> {
  ...
  const { data: profile } = await supabase
    ...
    .single();
  if (profile) {
    const p = profile as { experience_level?: string; subscription_tier?: string };
  ...
  const result: SessionContextWithTier = {
...
export function buildInterviewerInstructions(ctx: SessionContext): string {
